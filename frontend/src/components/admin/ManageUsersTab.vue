...
const showConfirm = (type: 'ban' | 'unban' | 'removeAvatar', userId: number) => {
  ...
  if (type === 'ban') confirmMessage.value = t('admin.confirmBan')
  else if (type === 'unban') confirmMessage.value = t('admin.confirmUnban')
        ...
        <tbody>
          <tr v-for="u in users" :key="u.id">
            ...
            <td class="cell-actions">
              <button class="action-link" @click="startEdit(u.id, u.username)">
                {{ $t('admin.editAction') }}
              </button>
              <button
                v-if="u.status === 1"
